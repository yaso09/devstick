...
from arch import get_regs_class, syscall_number, syscall_args, set_return_value
...
def read_string(pid: int, addr: int) -> str:
    """Tracee'nin belleğinden null-terminated string oku."""
    result = bytearray()
    while True:
...
def write_string(pid: int, addr: int, s: str):
    """Tracee belleğine string yaz (POKEDATA ile, 8 byte hizalı)."""
    data = s.encode("utf-8") + b"\x00"
    # 8 byte'a hizala
    padded = data + b"\x00" * (8 - len(data) % 8)
    for i in range(0, len(padded), 8):
        chunk = int.from_bytes(padded[i:i+8], "little")
        libc.ptrace(5, pid, ctypes.c_void_p(addr + i), ctypes.c_void_p(chunk))  # POKEDATA

def allocate_string(pid: int, regs, s: str) -> int:
    """
    Stack'te yer aç ve string'i yaz.
    ARM64: sp, x86_64: rsp
    """
    import platform
...
class Tracer:
    def __init__(self, rootfs: str, argv: list, bindings: dict = None):
        ...
        self.argv = argv
        self.in_syscall = {}   # pid → bool (syscall entry mi exit mi)
    ...
    def _handle_syscall(self, pid: int):
        ...
        if not is_entry:
            return   # exit: dönüş değerini değiştirmek istersen burada
        ...
        if name in ("getuid","geteuid","getgid","getegid"):
            ...
            # Syscall'ı iptal et: nr'yi geçersiz yap
            # (mimari bağımlı — basitlik için burada bırakıyoruz)
            return

        # chroot: yoksay, zaten rootfs içindeyiz
        if name == "chroot":
            set_return_value(pid, regs, 0)
            return
        ...
        for idx in path_arg_indices:
            ...
            # Yeni path'i stack'e yaz ve argümanı güncelle
